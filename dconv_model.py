...
class Block(nn.Module):
    # Block对应模块3*3 conv+relu+3*3 conv
    ...
    def forward(self, x):
        ...
        return x
    # 在forward函数中，x首先通过一个卷积层（self.conv1）和一个激活函数（self.op1），
    # 然后再通过另一个卷积层（self.conv2）和另一个激活函数（self.op2）。
    # 这个过程对应于模块的结构：33 conv + leakyrelu + 33 conv。
...
class EncoderBlock(nn.Module):
    ...
    # 分别对应（与paper中fig.3）：block--3*3 conv+relu+3*3 conv; nn.conv2d--strided conv步长=2>1;
...
class SCAM(nn.Module):
    # 立体交叉注意力模块Stereo Cross Attention Module在decoder中，融合distill block信息+skip connection信息，用于增强低分辨率图像的纹理
    def __init__(self, c):
        super().__init__()
        self.scale = c ** -0.5
...
class InnerMapBlock(nn.Module):
    ...
    def forward(self, x, key_info):
        # distill block传播函数，看return那个公式与fig.3中乘法和加法正好符合
        x_att = self.cam(x)
